  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc)
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){ //if child is killed (terminated), any child can be killed
        // Found one.
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        if(p->priority < low_val_prior_proc->priority || low_val_prior_proc->state != RUNNABLE) {
    ...
    */
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        if(p != min_priority_proc) { p->priority = p->priority - 1; }
    }
    min_priority_proc->priority = min_priority_proc->priority + 1;
